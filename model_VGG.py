...
import numpy as np
from Basic_function import conv_layer,Max_pooling,Fully_connected,Relu,Avg_pooling,Dropout,Global_Average_Pooling,Batch_Normalization
import random
class VGG:
    ...
    def __init__(self,input_shape,dataset,batch_size,epoch,learning_rate):
        ...
        self.checkpoint_path='./save/VGG'+dataset+'Epoch'+str(self.epoch)+'.ckpt'
        if dataset=="Cifar":
            self.output_num=10
    ...
    def model_structure(self,training):
        ...
        self.BN_1=Batch_Normalization(self.conv_2, training, scope="BN_1")
        self.Relu_2 = Relu(self.BN_1)

        self.conv_3 = conv_layer(self.Relu_2, filter=128, kernel=[3, 3], stride=1, padding='SAME', layer_name="conv_3")
        self.BN_2 = Batch_Normalization(self.conv_3, training, scope="BN_2")
        self.Relu_3 = Relu(self.BN_2)
        self.Dropout_3 = Dropout(self.Relu_3, self.keep_prob)


        self.Pool_0 = Max_pooling(self.Dropout_3,pool_size=[2, 2], stride=2, padding='VALID')

        self.conv_4 = conv_layer(self.Pool_0, filter=256, kernel=[3, 3], stride=1, padding='SAME', layer_name="conv_4")
        self.BN_3 = Batch_Normalization(self.conv_4, training, scope="BN_3")
        self.Relu_4 = Relu(self.BN_3)

        self.conv_5 = conv_layer(self.Relu_4, filter=256, kernel=[3, 3], stride=1, padding='SAME', layer_name="conv_5")
        self.BN_4 = Batch_Normalization(self.conv_5, training, scope="BN_4")
        self.Relu_5 = Relu(self.BN_4)
        self.Dropout_4 = Dropout(self.Relu_5, self.keep_prob)
    ...
    def fit(self):
        ...
        with tf.Session() as sess:
            ...
            for j in range(10000000):
                ...
                train_step.run(feed_dict={self.img:img,self.label:lbl,Learning_rate:self.learning_rate,self.keep_prob:0.5})
                Probability=Output_Probability.eval(feed_dict={self.img:img,self.label:lbl,self.keep_prob:1.0})
                Probability=np.asarray(Probability)

                if self.output_num == 1:
                    assert np.shape(Probability) == (self.batch_size, 1)
                    train_accuracy = train_accuracy + np.mean(
                        (Probability > 0.5) * lbl + (Probability <= 0.5) * (1 - lbl))
